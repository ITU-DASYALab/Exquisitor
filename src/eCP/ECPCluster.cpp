...
ECPCluster<T,U,V>::ECPCluster(ECPConfig *_cnfg, FILE *_indxfile, FILE *_datafile)
{
    // Note the bookkeeping info for later
    cnfg = _cnfg;
    datafile = _datafile;
    descriptorSize = this->getDescriptorSize(); //ATH breyta
    ...
    //descriptorList = new Descriptor*[descriptors];
    descriptorIds = (uint32_t*) malloc(sizeof(uint32_t) * descriptors);
    // Prepare for scans
    nextDescriptor = -1;
};
...
ExqDescriptor<T,U,V>* ECPCluster<T,U,V>::next()
{
    if (nextDescriptor == descriptors) 
        return NULL;
    nextDescriptor++;
    return new ExqDescriptor<T,U,V>(datafile, 1);
};
