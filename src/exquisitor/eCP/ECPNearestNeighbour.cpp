...
#endif
    extendedValueArr = vector<float>(_featureDimensions);
    for(uint32_t i = 0; i < (uint32_t)_featureDimensions; i++) {
        extendedValueArr[i] = 0;
    }

    indexQueryArr = vector<vector<int>>();
    for(uint32_t y = 0; y < 2; y++) {
        indexQueryArr.push_back(vector<int>(_functions->getDescFeatCount()));
...
float ECPNearestNeighbour::Distance(ExqDescriptorR64* data) {
    float dist = 0.0;
...
void ECPNearestNeighbour::FindFarthest() {
    ...
    float dist = distances[neighbors-1];
    for (int i = neighbors-2; i >= 0; --i) {
        if (distances[i] > dist) { //now we only change order if the dist of new neigbor is LESS than the other distances
            float  tdis        = distances[i];
            int    tid         = descriptorIDs[i];
            uint32_t tcl         = clusterIDs[i];
            // move forward
