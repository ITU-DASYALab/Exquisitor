...
As an example, imagine that one needs to create an HDF5 file named "myFile.h5"
and, inside it, a group named "myGroup" containing a (ZLIB) compressed dataset
named "myDataset" of data type float with an initial value of 12.4. In HDFql,
...
In contrast, using the C API on the same example is quite cumbersome:
   ...
   hsize_t dimension;
   float value;
   file = H5Fcreate("myFile.h5", H5F_ACC_EXCL, H5P_DEFAULT, H5P_DEFAULT);
   ...
   dataset = H5Dcreate(group, "myDataset", H5T_NATIVE_FLOAT, dataspace, H5P_DEFAULT, property, H5P_DEFAULT);
   value = 12.4;
