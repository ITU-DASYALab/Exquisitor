...
def variable_register(numpy_array):
	...
	if numpy_array.dtype.type not in [_numpy.int8, _numpy.uint8, _numpy.int16, _numpy.uint16, _numpy.int32, _numpy.uint32, _numpy.int64, _numpy.uint64, _numpy.float32, _numpy.float64, _numpy.string_, _numpy.ubyte]:
	...
	for i in (0, 1, 2, 3, 4, 5, 6, 7):
		if _variable_list[i] is None:
			...
			if address == _variable_list[i]:
				_HDFql.variable_register(i, address, _HDFql.NO)
				return i
	if number == -1:
		return _HDFql.ERROR_FULL
	_HDFql.variable_register(number, address, _HDFql.NO)
...
def variable_transient_register(numpy_array):
	...
	for i in (0, 1, 2, 3, 4, 5, 6, 7):
		if _variable_list[i] is None:
			...
			if address == _variable_list[i]:
				_HDFql.variable_register(i, address, _HDFql.YES)
				return i
	if number == -1:
		return _HDFql.ERROR_FULL
	_HDFql.variable_register(number, address, _HDFql.YES)
	_variable_list[number] = address
	return number
